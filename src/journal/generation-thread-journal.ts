...
const makeBindGenerationThread =
  ...
  (generationId, threadId) =>
    tryJournalTransaction('bindGenerationThread', 'bindGenerationThread failed', () => {
      const result = database.run(
        'UPDATE generations SET codex_thread_id = ? WHERE id = ? AND codex_thread_id IS NULL',
        [threadId, generationId],
      );
      if (result.changes !== 1) {
        throw new Error(`bindGenerationThread expected one row, changed ${String(result.changes)}`);
      }
    });
...
const makeLoadGenerationThreadState =
  ...
  (generationId) =>
    tryJournalTransaction('loadGenerationThreadState', 'loadGenerationThreadState failed', () => {
      const row = database
...
const makeReplaceUnusedGenerationThread =
  ...
  (generationId, expectedThreadId, replacementThreadId) =>
    tryJournalTransaction(
      'replaceUnusedGenerationThread',
      'replaceUnusedGenerationThread failed',
      () => {
        const result = database.run(
