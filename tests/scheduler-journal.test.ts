...
it.effect('reconstructs pooled timing and acknowledgement state after restart', () =>
  Effect.gen(function* schedulerJournalFixture() {
    ...
    yield* journal.beginTurn(initial.generationId, logicalTurnId, [turnMessage], now);
    yield* journal.save(
      {
        ...initial,
    ...
    yield* journal.recordAcknowledgement(logicalTurnId, new Date(now.getTime() + 2000));
    ...
    yield* journal.save(
      { ...restarted, active: null, generationBroken: true },
    ...
    yield* journal.resetGeneration(resetState, commandAt, commandId);
