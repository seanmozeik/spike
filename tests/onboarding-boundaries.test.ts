...
const messagesFixture = (): string => {
  ...
  database.run("INSERT INTO chat VALUES (1, 'direct-email', 45), (2, 'group', 45)");
  database.run('INSERT INTO chat_handle_join VALUES (1, 1), (2, 1), (2, 2)');
...
it('discovers only exact style-45 conversations with one participant', () => {
  expect(discoverDirectConversations(messagesFixture(), 'spike@icloud.com')).toMatchObject([
